...
const Sellers = () => {
  ...
  const pendingCount = sellerList.filter((s) => s.verificationStatus === 'pending').length;
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="text-sm text-gray-600 bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm font-medium">
          Total Stores: <span className="font-bold text-gray-900">{pagination.total || pagination.totalResults || sellerList.length}</span>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="bg-gray-50 text-gray-600 font-semibold border-b border-gray-200 text-xs">
                  ...
                  <th className="py-3.5 px-4">Location (2dsphere)</th>
                  <th className="py-3.5 px-4">Tax IDs</th>
                  <th className="py-3.5 px-4">KYC Status</th>
                  <th className="py-3.5 px-4">Suspension</th>
                ...
                {sellerList.map((s) => (
                  <tr key={s._id} className="hover:bg-gray-50/75 transition-colors">
                    <td className="py-3.5 px-4">
                      <div className="font-bold text-gray-900 text-sm">{s.shopName}</div>
                      <div className="text-xs text-gray-500">
                        {s.shopAddress?.city || 'No City'}, {s.shopAddress?.pincode || ''}
                      </div>
                    </td>
                    <td className="py-3.5 px-4">
                      <div className="font-medium text-gray-900">{s.user?.name || 'Seller'}</div>
                      <div className="text-xs text-gray-500">{s.user?.email || 'N/A'}</div>
                      {s.user?.phone && (
                    ...
                    <td className="py-3.5 px-4">
                      {s.location?.coordinates ? (
                        <div className="font-mono text-xs text-gray-700 bg-gray-50 px-2 py-1 rounded inline-block border border-gray-200">
                    ...
                    <td className="py-3.5 px-4 text-xs">
                      <div>GST: <span className="font-mono">{s.gstNumber || 'N/A'}</span></div>
                      <div>PAN: <span className="font-mono">{s.panNumber || 'N/A'}</span></div>
                    </td>
                    <td className="py-3.5 px-4">
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {s.verificationStatus === 'approved' && (
                          <Button
                            ...
                            size="sm"
                            className={!s.isSuspended ? 'text-amber-600 hover:text-amber-700 hover:bg-amber-50' : ''}
        ...
        {selectedSeller && (
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-gray-200">
              <div>
                <h3 className="font-bold text-gray-900 text-lg">{selectedSeller.shopName}</h3>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <span className="text-xs text-gray-500 block">Shop Address</span>
                <span className="text-gray-800">
                  {selectedSeller.shopAddress?.line1}, {selectedSeller.shopAddress?.city}, {selectedSeller.shopAddress?.pincode}
                </span>
              </div>
              <div>
                <span className="text-xs text-gray-500 block">Coordinates (Lng, Lat)</span>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-3">
              ...
              <Button
                ...
                className={verifyAction === 'approved' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}
              >
            ...
            {!selectedSeller.isSuspended && (
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-300 p-2 text-sm focus:ring-2 focus:ring-amber-500 outline-none"
                />
              </div>
            )}
